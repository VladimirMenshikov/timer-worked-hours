...
Этот каталог (`timer/`) — часть моно-репозитория `ai-timer-worked`. Общий
установщик и миграции — на уровень выше, в корне репозитория.

```
ai-timer-worked/
├── install.sh          — единый установщик (Postgres + миграции + timer + dashboard)
├── sql/migrations/     — общие для timer/ и dashboard/ файлы миграций
└── timer/
    ├── timer.py          — основное приложение
    ├── db_backend.py     — доступ к БД (Supabase REST или прямой PostgreSQL) и автомиграции
    ├── .env              — данные подключения к БД (заполняется install.sh или мастером первого запуска)
    ├── requirements.txt  — Python-зависимости
    ├── install.sh        — установка только python-зависимостей (для повторной настройки)
...
Если задан `DATABASE_URL` (в любом из двух режимов), таблицы создаются **автоматически**: при каждом запуске приложение проверяет общий каталог `../sql/migrations/` и применяет ещё не применённые файлы по порядку. При установке через корневой `install.sh` это происходит один раз при установке.

Без `DATABASE_URL` (только Supabase REST, без доступа к прямому SQL) — выполните миграции вручную по порядку в **Supabase → SQL Editor**:
